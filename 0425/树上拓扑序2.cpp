...
int n, dp[N], f[N], siz[N], fac[N], inv[N];
std::vector<int> e[N];
...
void dfs(int x, int fa) {
    ...
    for (int y : e[x])
        if (y != fa) {
            dfs(y, x);
            dp[x] = mul(dp[x], mul(dp[y], binom(siz[x] + siz[y] - 1, siz[x] - 1)));
            siz[x] += siz[y];
        }
    return void();
}
...
void dfs2(int x, int fa) {
    for (int y : e[x])
        if (y != fa) {
            f[y] = mul(mul(mul(f[x], qpow(mul(dp[y], binom(n - 1, n - siz[y] - 1)), P - 2)), binom(n - 1, n - siz[y])), dp[y]);
            dfs2(y, x);
        }
    return void();
}
...
signed main() {
    ...
    f[1] = dp[1];
    dfs2(1, 0);
    for (int i : Range<int>(1, n)) fprintf(fout, "%d\n", f[i]);
    return 0;
}